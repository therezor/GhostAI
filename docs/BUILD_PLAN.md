...
- **`ghost serve` enumerates the UI directory at boot** (`@fastify/static` with `wildcard: false`), so a rebuild while the server is running serves the new `index.html` and 404s its hashed assets into the SPA fallback — a blank page that looks like a crash. Restart after a UI build. **Worth a dev-mode note in Step 19.**
