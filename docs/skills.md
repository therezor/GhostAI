...
There is **no `skill` tool**, and that is a decision. A tool whose whole job is to return
the bytes of a workspace file is a worse `read_file` — one more name in every agent's
permission map and one more schema in every request, to reach a file the agent can already
open. See [Tools](tools.md), which opens with the same argument about why there are six
built-ins.
...
because `packages/core/src/paths.ts` argues the opposite case for an agent's _memory_ and
puts it in `~/.ghostai/agents/<id>/` — outside the jail — precisely so that prompt
injection cannot rewrite what the agent believes.
