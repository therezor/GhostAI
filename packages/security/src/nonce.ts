...
import { effectiveToolPolicy, renderPromptTemplate } from '@ghostwire/protocol';
 ...
 *
 * The default lives in `@ghostwire/protocol` rather than here so the browser can
...
export function toolOutputPolicy(
  ...
  // cached half of the prompt — `toolOutputTag` needs a real nonce and throws
  // without one, so computing it eagerly would have made `nonce` mandatory for a
