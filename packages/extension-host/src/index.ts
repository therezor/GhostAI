/**
 * @ghostwire/extension-host — third-party code, authorised and loaded.
 *
 ...
 * asks `@ghostwire/security` whether the bytes on disk are the ones an operator
 ...
 * package rather than a module in `@ghostwire/runtime`: an extension may
 * contribute a `ChannelFactory`, so something has to name `@ghostwire/channels`,
 * and `runtime` has no business importing it — channels are wired in
 * `ghostai serve`, one layer further out again.
 *
 ...
 *    `@ghostwire/security`'s `extension.ts`, and it is the difference between
