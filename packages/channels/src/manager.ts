...
function toFrameContent(content: readonly ContentPart[]): {
  ...
  for (const part of content) {
    ...
    texts.push(
      `[image omitted: ${part.mimeType} — publish a file part instead]`,
    );
  }
...
export class ChannelManager {
  ...
  private readonly channelsConfig: Readonly<Record<string, unknown>>;
  private readonly projectionOptions: TurnProjectionOptions;
  ...
  private readonly bridged = new Map<string, Bridged>();
  private readonly lifetime = new AbortController();
  private pumps: Promise<void> | undefined;
  ...
  constructor(options: ChannelManagerOptions) {
    ...
    this.messageBus = options.bus ?? new MessageBus(options.busOptions ?? {});
  ...
  channel(id: string): Channel | undefined {
    return this.channelsByName.get(id);
  }
  ...
  register(factory: ChannelFactory): void {
    if (this.started) {
      throw new Error(
        `Channel "${factory.id}" was registered after the manager started`,
      );
    }
  ...
  async start(): Promise<void> {
    ...
    try {
      for (const factory of this.factories.values()) {
        ...
        if (flagOf(settings, 'enabled') === false) {
          this.logger.info(
            { channel: factory.id },
            'channel disabled by config',
          );
          continue;
        }
        const channel = await factory.create(
          this.context(factory.id, settings),
        );
        ...
        await channel.start?.();
        this.channelsByName.set(factory.id, channel);
      }
    } catch (error) {
      await this.stop();
      throw error;
    ...
    this.pumps = Promise.all([this.pumpInbound(), this.pumpOutbound()]).then(
      () => undefined,
    );
    this.logger.info(
      { channels: [...this.channelsByName.keys()] },
      'channels started',
    );
  }
  ...
  async stop(): Promise<void> {
    this.lifetime.abort();
    if (this.ownsBus) this.messageBus.close();
    await this.pumps;
  ...
  private async pumpInbound(): Promise<void> {
    for await (const message of this.messageBus.inbound()) {
      try {
        this.toHub(message);
      } catch (error) {
  ...
  private toHub(message: InboundMessage): void {
  ...
  private sessionKey(channelId: string, sessionKey: string): string {
  ...
  private fromHub(key: string, event: ServerMessage): void {
    ...
    for (const draft of bridged.projection.project(event)) {
      const result = this.messageBus.publishOutbound({
  ...
  private async pumpOutbound(): Promise<void> {
    for await (const message of this.messageBus.outbound()) {
  ...
  private dispatch(message: OutboundMessage): void {
    const channel = this.channelsByName.get(message.channelId);
    ...
    const accepts: readonly OutboundKind[] =
      channel.accepts ?? DEFAULT_ACCEPTED_KINDS;
