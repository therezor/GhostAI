...
import { agentForTurn } from '#src/agent-binding.js';
...
describe('agentForTurn', () => {
  it('takes the frame’s pick for a session that does not exist yet', () => {
    expect(
      agentForTurn({ stored: undefined, requested: 'reviewer', resolves }),
    ).toBe('reviewer');
    expect(agentForTurn({ stored: '', requested: 'reviewer', resolves })).toBe(
      'reviewer',
    );
  });
  ...
  it('leaves an unbound session unbound when the frame names nobody', () => {
    expect(
      agentForTurn({ stored: undefined, requested: undefined, resolves }),
    ).toBeUndefined();
  });
  ...
  it('keeps the stored agent even when the frame names another', () => {
    ...
    expect(
      agentForTurn({ stored: 'writer', requested: 'reviewer', resolves }),
    ).toBe('writer');
  });
  ...
  it('lets the frame win when the stored agent no longer resolves', () => {
    ...
    expect(
      agentForTurn({ stored: 'deleted', requested: 'reviewer', resolves }),
    ).toBe('reviewer');
  });
  ...
  it('keeps the stored agent when neither resolves', () => {
    ...
    expect(
      agentForTurn({ stored: 'deleted', requested: 'deleted', resolves }),
    ).toBe('deleted');
    expect(
      agentForTurn({ stored: 'deleted', requested: undefined, resolves }),
    ).toBe('deleted');
  });
});
