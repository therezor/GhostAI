...
import { AuthStore, argon2Hasher, type PasswordHasher } from '#src/auth-store.js';
...
describe('passwords', () => {
  ...
  it('refuses a password below the minimum rather than storing a hash of it', async () => {
    const { store } = build();
    await expect(store.setPassword('')).rejects.toThrow(/at least 12 characters/);
    await expect(store.setPassword('short')).rejects.toThrow(/at least 12 characters/);
...
describe('the username', () => {
  ...
  it('refuses a name the login route would have refused', async () => {
    const { store } = build();
    await expect(store.setPassword('a good password', 'has spaces')).rejects.toThrow(/username/i);
  ...
  it('refuses a password that is the username', async () => {
    const { store } = build();
    await expect(store.setPassword('operatorname', 'operatorname')).rejects.toThrow(
  ...
  it('will not be handed out or generated by ensureSecret', () => {
    const { store } = build();
    expect(() => store.ensureSecret('username')).toThrow(/not a readable secret/);
...
describe('verifyLogin', () => {
  ...
  it('refuses a name too malformed to normalise, without throwing', async () => {
    ...
    expect(await store.verifyLogin('has spaces', 'a good password')).toBe(false);
  });
