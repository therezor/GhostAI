...
import { cookieSecure, readCredential, SESSION_COOKIE } from '#src/auth.js';
...
describe('readCredential', () => {
  ...
  it('reads a bearer token', () => {
    expect(
      readCredential(request({ headers: { authorization: 'Bearer abc.def' } })),
    ).toBe('abc.def');
  });
  ...
  it('ignores another scheme entirely', () => {
    expect(
      readCredential(
        request({ headers: { authorization: 'Basic dXNlcjpwdw==' } }),
      ),
    ).toBeUndefined();
  });
  ...
  it('ignores an empty bearer value', () => {
    expect(
      readCredential(request({ headers: { authorization: 'Bearer   ' } })),
    ).toBeUndefined();
  });

  it('reads the session cookie', () => {
    expect(
      readCredential(request({ cookies: { [SESSION_COOKIE]: 'abc.def' } })),
    ).toBe('abc.def');
  });

  it('ignores an empty cookie', () => {
    expect(
      readCredential(request({ cookies: { [SESSION_COOKIE]: '' } })),
    ).toBeUndefined();
  });
...
describe('cookieSecure', () => {
  it('is set over HTTPS regardless of host', () => {
    expect(
      cookieSecure(request({ protocol: 'https', hostname: '127.0.0.1' })),
    ).toBe(true);
  });
  ...
  it.each(['127.0.0.1', 'localhost', '::1'])(
    'is not set over plain HTTP to %s',
    (hostname) => {
