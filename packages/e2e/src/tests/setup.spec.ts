...
test.describe('an unclaimed install', () => {
  ...
  test('takes the one-time code and lands in the app', async ({ page, harness }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Add a model provider' })).toBeVisible();
    await page.getByRole('button', { name: 'Skip' }).click();
    await page.getByRole('button', { name: 'Skip' }).click();
  ...
  test('refuses a wrong code and stays on the step', async ({ page, harness }) => {
    ...
    await expect(page.getByRole('alert')).toContainText('Incorrect or already-used');
    await expect(page.getByRole('heading', { name: 'Enter the setup code' })).toBeVisible();
  });
...
test.describe('a claimed install with no model', () => {
  test.use({ harnessOptions: { config: { agents: { defaults: { model: '' } } } } });

  test('opens at the provider step rather than asking for a spent code', async ({ app }) => {
    await expect(app.getByRole('heading', { name: 'Add a model provider' })).toBeVisible();
    await expect(app.getByRole('heading', { name: 'Enter the setup code' })).toBeHidden();
  ...
  test('leaves every other screen working, and says why chat is not', async ({ app }) => {
    await app.getByRole('button', { name: 'Skip' }).click();
    await app.getByRole('button', { name: 'Skip' }).click();

    const composer = app.getByRole('textbox', { name: 'Message' });
    await expect(composer).toBeDisabled();
    await expect(composer).toHaveAttribute('placeholder', 'No model configured yet');
    await expect(app.getByRole('link', { name: 'Add a provider' })).toBeVisible();
