 ...
 *
 * `self-contained.test.ts` in `@ghostai/web` already fails on any external
...
test.describe('with every foreign origin blocked', () => {
  test('the whole app still works', async ({ page, harness }) => {
    ...
    const loaded = await page.evaluate(() => document.fonts.check('1rem Inter'));
    expect(loaded, 'the bundled Inter should have loaded from this origin').toBe(true);
    ...
    expect(blocked, 'nothing should have been requested from another origin').toEqual([]);
  });
});
